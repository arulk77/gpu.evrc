...
/* Define functions to get cosine and sine from phs_tbl */
/* These functions compute cos(2*PI*(arg-16384)/64) and
 * sin(2*PI*(arg-16384)/64) for arg in the range [16362, 16416).
 * Since they are used with random arguments they produce the
 * expected results for arg in the range [0, 32768).
 */
Word16 cos64_fx(Word16 arg)
  {
  Word16 result;

  result = arg & 0x007e;	/* Get bottom 7 bits, mask off last bit */
  result = phs_tbl[result];
  if (arg > 0x3fff)
    {
    result = negate(result);	/* Table contains -sin(x) */
    }
  return (result);
  }


Word16 sin64_fx(Word16 arg)
  {
  Word16 result;

  result = arg & 0x007e;	/* Get bottom 7 bits, mask off last bit */
  result = add(result, 1);	/* Sine table uses odd indices */
  result = phs_tbl[result];
  if (arg < 0x4000)
    {
    result = negate(result);	/* Table contains -sin(x) */
 ...
 */
void c_fft(Word16 * farray_ptr, Word16 isign)
{
...
void r_fft(Word16 * farray_ptr, Word16 isign)
{
