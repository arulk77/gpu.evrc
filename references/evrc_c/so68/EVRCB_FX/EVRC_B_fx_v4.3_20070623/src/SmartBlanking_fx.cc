...
#define LOW_LSP_40P_THRESHOLD               35  //was 71 in float
#define HIGH_LSP_40P_THRESHOLD              39  //was 79 in float
    ...
    for (i=0; i < LSP_CODEBOOK_SIZE; i++)    // 16 codebooks
    {
        if (i == LSPs)
        {
            //Histogram[i] = (Histogram[i] * (100 - FILTER_CONSTANT) + 100 * FILTER_CONSTANT)/100;
              //Histogram[i] = 5 + (0.95 * Histogram[i]);
			  temp = (L_mult(31128,Histogram[i]));
			  Histogram[i] = add(5,extract_h(temp));
              
        }
        else
        {
            //Histogram[i] =  (Histogram[i] * (100 - FILTER_CONSTANT))/100;
			//Histogram[i] = 0.95 * Histogram[i];
			 temp = (L_mult(31128,Histogram[i]));
             Histogram[i] = extract_h(temp);
        }
    }
}
...
void SBEncoder::UpdateFilteredPower(unsigned char Power)
{
    ...
    //    FilteredPower = (FilteredPower * 90 +  EVRCEigthRatePower[Power] * 10)/100;
	      temp = L_add(L_mult(29490,FilteredPower), L_mult(3276,EVRCEigthRatePower[Power]));
		  FilteredPower = extract_h(temp);
        break;
    case FGV_VOCODER :
    //  	FilteredPower = (FilteredPower * 80 +  FGVEigthRatePower[Power>>2] * 20)/100;
       temp1 = shr(Power,2);
       temp = L_add(L_mult(26213,FilteredPower), L_mult(6553,EVRCEigthRatePower[temp1]));
	   //printf("%d \n",(temp>>16));
	   FilteredPower = extract_h(temp);
        break;
    }
...
int SBEncoder::GetDeltaFromAverage(unsigned char Power)
{
    ...
    case EVRC_VOCODER :
        //DeltaFromAverage = abs(FilteredPower - EVRCEigthRatePower[Power]);
		  ...
		  DeltaFromAverage = abs_s(sub(FilteredPower, EVRCEigthRatePower[Power]));
        break;
    case FGV_VOCODER :
        //DeltaFromAverage = abs(FilteredPower - FGVEigthRatePower[Power>>2]);
        temp = shr(Power,2);
		DeltaFromAverage = abs_s(sub(FilteredPower, EVRCEigthRatePower[temp]));
        break;
    }
...
SBEncoder::Encode(short* Rate, unsigned char* IBuffer, short refl_flag,short nsid)
{
    ...
    else
    {
        ...
        if (FirstTime)
        {
            ...
            ProtoLowLSPs = shr(EigthRatePrototype[LSPS_BYTE_POS],4);;
          ...
          //  FilteredPower = EVRCEigthRatePower[EigthRatePrototype[ENERGY_BYTE_POS]>>2];
            temp = shr(EigthRatePrototype[ENERGY_BYTE_POS],2);
			FilteredPower = FGVEigthRatePower[temp];
...
//    if (InSilence > 1)    // Number of frames to send
    {
        *Rate = BLANK_RATE;
        NFramesSinceLastSent=add(NFramesSinceLastSent,1);
    }
    else
    {
    ...
    if ((InSilence > N_TRANSITORY_FRAMES) || (refl_flag==1))  // number after which encoder is assumed stable, =5
    {
        ...
        LowLSPs = shr(Current8Frame[LSPS_BYTE_POS], 4);
        ...
        temp1 = L_add(L_mult(31128,FilteredLowLSPsDeltas),L_mult(1638,EVRCLowLSPsDistance[LowLSPs][ProtoLowLSPs]));
		//printf("%d \n",(temp1>>16));
		FilteredLowLSPsDeltas = extract_h(temp1);
        ...
        temp1 = L_add(L_mult(31128,FilteredHighLSPsDeltas),L_mult(1638,EVRCHighLSPsDistance[HighLSPs][ProtoHighLSPs]));

		//printf("%d \n",(temp1>>16));
		FilteredHighLSPsDeltas = extract_h(temp1);
	...
	if (nsid==0)
	 {
		...
		temp = shl(LowLSPs ,4);
		CandidateEigthRatePrototype[LSPS_BYTE_POS] = (temp | HighLSPs);
