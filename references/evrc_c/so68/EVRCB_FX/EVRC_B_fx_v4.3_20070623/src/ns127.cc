...
/* Export pointer to window for use in blind_bg.cc */
Word16 BBG_window_FIX[DELAY + FRM_LEN] =
	{
...
/* These are now globals so they can be shared by blind_bg.cc */
Word16 ch_tbl[NUM_CHAN][2] =
{
...
Word16 block_norm(Word16 *data, Word16 size, Word16 headroom)
{
...
void block_denorm(Word16 * data, Word16 size, Word16 scnt)
{
...
void noise_suprs_fx(Word16 * farray_ptr, Word16 *ns_snr)  // ns_snr in Q9
{
 ...
 */
	...
	static Word16 ch_tbl_sh[NUM_CHAN][2] =
	{
...
#if !MOT_SUPPRESSED_EIGHTH_RATE
	static Word16 window[DELAY + FRM_LEN] =
	{
...
#else
/* Export pointers to window for use in blind_bg.cc */
	const static Word16 *window = BBG_window_FIX;
#endif

	static Word16 first = TRUE;
	static Word16 pre_emp_mem;
	static Word16 de_emp_mem;
	static Word16 overlap[FFT_LEN - FRM_LEN];
	static Word16 ch_gain[FFT_LEN / 2];	/* scaled as 0,15 */
	static Word16 update_cnt;
	static Word16 window_overlap[DELAY];
	static Word16 hyster_cnt;
	static Word16 last_update_cnt;
	static Word16 ch_enrg_long_db[NUM_CHAN];		/* scaled as 10,5  */

	static Word32 frame_cnt;
	static Word32 ch_enrg[NUM_CHAN];	/* scaled as 23,8 */
	static Word32 ch_noise[NUM_CHAN];		/* scaled as 15,16 (change to 23,8) */

	static Word16 last_normb_shift;		/* last block norm shift count */

	Word32 enrg;				/* scaled as 30,1 */
	Word32 tne;				/* scaled as 15,16 (change to 23,8) */
	Word32 tce;				/* scaled as 23,8 */
	Word32 gain;				/* scaled as 11,20 */

	Word16 data_buffer[FFT_LEN];
	Word16 ch_snr[NUM_CHAN];	/* scaled as 15,0 */
	Word16 ftmp2;			/* scaled as 0,15 */
	Word16 vm_sum;			/* scaled as 15,0 */
	Word16 ch_enrg_dev;		/* scaled as 10,5 */
	Word16 ch_enrg_db[NUM_CHAN];		/* scaled as 10,5 */
	Word16 alpha;			/* scaled as 0,15 */

	int i, j, j1, j2;
	int update_flag, modify_flag, index_cnt;

	Word32 Ltmp, Ltmp1, Ltmp2, Ltmp3;
	Word16 tmp, tmp1, tmp2, norm_shift, norm_shift1;
...
/* Compute the total noise estimate (tne) and total channel energy estimate (tce) */
  ...
  for (i=LO_CHAN;i<11;i++) {
    band_eng=L_add(band_eng,ch_enrg[i]);  // Q8
    band_noise=L_add(band_noise,ch_noise[i]); // Q8
  }
  band_eng=log10_lut(band_eng); // output in Q23
  band_noise=log10_lut(band_noise); // Q23
  band_eng=L_sub(band_eng,band_noise);
  ns_snr[0]=MAX_FX(0,round32_16(L_shl(band_eng,2))); // Q9
  ...
  for (i=11;i<=HI_CHAN;i++) {
    band_eng=L_add(band_eng,ch_enrg[i]);
    band_noise=L_add(band_noise,ch_noise[i]);
  }
  band_eng=log10_lut(band_eng); // output in Q23
  band_noise=log10_lut(band_noise); // Q23
  band_eng=L_sub(band_eng,band_noise);
  ns_snr[1]=MAX_FX(0,round32_16(L_shl(band_eng,2))); // Q9
...
/* Calculate log spectral deviation */
  ...
  move16();
		...
		if (Ltmp == 0)
			Ltmp = 1;
		Ltmp1 = fnLog10(Ltmp);
		Ltmp2 = L_add(Ltmp1, LOG_OFFSET - 161614248);	/* -round(log10(2^8)*2^26) */
...
/* Compute the channel gains */
	Ltmp1 = fnLog10(tne);
	Ltmp1 = L_add(Ltmp1, LOG_OFFSET - 161614248);	/* -round(log10(2^8)*2^26) */
