...
#define L_GAIN_LIM       419430
#define GAIN_CONST       29309
#define GAIN_LEV         290
...
short sqrt_approx(long);

/*===========================================================================*/
/*         ..Scales input data block to avoid overflow in goertzel filters.  */
/*---------------------------------------------------------------------------*/
void  agc (
    DTMF_DETECTOR* D,     /* (i)   - pointer to detector structure */
    short*         data   /* (i/o) - data buffer */
)
{
     /* ..(local) variables..*/
     register  int j;
	 ...
	 for (j = 0; j < N; j++)
     {
         accA = L_mac40(accA, *dinP, *dinP);
         dinP++;
     }
     accA =L_shr40(accA,6);      /* A = A / 64 */
     Lgain_power =  accA ;
     dinP = data;
     doutP = data;
     if (Lgain_power> L_GAIN_LIM ) {
        accA = L_mult(GAIN_CONST, sqrt_approx(Lgain_power));

		accA=L_shr40(accA,16);
        gain_amp = extract_l40( (accA));
	 
        gain_scale = divide_s(GAIN_LEV,gain_amp);
        
        /* scale the data by 'gain_scale' */
        for (j = 0; j < N; j++)
        {
		
            accA = L_mult(gain_scale, (*dinP++));
		 	accA= L_shr40(accA,16);
		    *doutP++ =extract_l40(accA);
...
short sqrt_approx ( /* (o) - output: 0 <= y < 1.0 (Q15) */
    long  L_x       /* (i) - input:  0 <= x < 1.0 (Q31) */
)
{
     /* ..(local) variables..*/
     register  int j;
     ...
     /* ..execute..*/
     accC =  (0x40000000);             /* delta = 0.5 */
     y = 0x4000;                          /* y = 0.5 */

     accA = L_mult(y,y);
     accA = L_sub40(accA, L_x);          /* (A) = yold*yold - x */

     for (j=0; j<SQRT_ITERATIONS; j++) {
         accC = L_shr40(accC,1);          /* (1/2)*delta */
         accB = L_shl(y,16);          /* (B) = yold */
         if (accA > 0) {
             accB = L_sub40(accB, accC); /* (B) = yold - (1/2)*delta */
         } else {
             accB = L_add40(accB, accC); /* (B) = yold + (1/2)*delta */
         }
		 accB=L_shr40(accB,16);
         y = extract_l40(accB);              /* (BH) --> ynew */
